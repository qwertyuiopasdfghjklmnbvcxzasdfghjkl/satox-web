    ...
    <div class="twoverify-container">
      ...
      <div class="twoverify-right">
        <div class="twoverify-right-item item-login">
          ...
          <div class="verifyCode" :style="type === '01' || type === '10' ? null : 'margin-top:40px;'"
               @click="focusVerifyCode">
            <a href="javascript:" v-if="Number(formData.verifyType) === 0" class="sendVerifyCode"
               :class="{disabled:disabled}" @click="sendSMSCode">{{$t('account.user_center_send_SMS')}}<!--发送验证码-->
              {{disabled ? `（${time}s）` : ''}}</a>
            <inputbox name="verifyCode" ref="verifyCode" :maxLength="verifyCodeLength" v-model="formData.verifyCode"
                      v-validate="'required'" :msgs="msgs.verifyCode" :errs="errors" :title="codeTitle"/>
            <!--短信验证码||谷歌验证码-->
            <ul>
              <li :class="{active:index<=verifyCodes.length}" v-for="index in verifyCodeLength" :key="index">
                {{verifyCodes[index - 1]||''}}
              </li>
            </ul>
          </div>
...
<style scoped>
  ...
  .twoverify-right-item {
    position: relative;
    height: 520px;
    padding: 40px 60px;
  }

  .twoverify-right-item .caption {
    height: 40px;
    font-size: 20px;
    line-height: 40px;
    color: #3A76E7;
  ...
  .twoverify-right-item .tabs {
    display: flex;
    height: 45px;
    margin: 60px 0;
  }

  .twoverify-right-item .tabs label {
    display: flex;
    flex: 1;
    align-items: center;
    justify-content: center;
    font-size: 16px;
    color: #A1A8BB;
    border: 1px solid #3F4D6E;
    cursor: pointer;
  }

  .twoverify-right-item .tabs label.checked {
    border-color: #fdb902;
    color: #fdb902;
  }

  .twoverify-right-item .tabs label input {
    display: none;
  }

  .twoverify-right-item .prompt {
    padding-bottom: 24px;
    overflow: auto;
  }

  .twoverify-right-item .prompt i {
    float: left;
    width: 16px;
    height: 16px;
    margin-top: 12px;
  ...
  .twoverify-right-item .prompt span {
    float: left;
    width: 404px;
    min-height: 24px;
  ...
  .twoverify-right-item /deep/ .inputbox {
    width: auto !important;
    min-height: auto !important;
  }

  .twoverify-right-item /deep/ .title {
    height: 34px;
    line-height: 34px;
    font-size: 16px;
    color: #333;
  ...
  .twoverify-right-item /deep/ .inputdiv {
    position: relative;
    display: block;
    padding-bottom: 24px;
  }

  .twoverify-right-item /deep/ .input {
    width: 100% !important;
    height: 40px;
    margin: 0;
    padding: 0;
  ...
  .twoverify-right-item /deep/ .input:focus {
    background-color: transparent;
  }

  .twoverify-right-item /deep/ .input.error {
    background-color: transparent;
    border-bottom-color: #e53f3f;
  }

  .twoverify-right-item /deep/ .errorinfo {
    position: absolute;
    bottom: 0;
    left: 0;
    width: auto;
  ...
  .twoverify-right-item .captcha {
    overflow: auto;
  }

  .twoverify-right-item .captcha /deep/ .inputbox {
    float: left;
    width: 280px !important;
  }

  .twoverify-right-item .captcha img {
    float: right;
    margin-top: 34px;
    cursor: pointer;
  }

  .twoverify-right-item .checkbox-group {
    overflow: auto;
  }

  .twoverify-right-item .checkbox-group i {
    float: left;
    width: 14px;
    height: 14px;
    margin-top: 8px;
  ...
  .twoverify-right-item .checkbox-group i:hover {
    color: #fdb902;
  }

  .twoverify-right-item .checkbox-group span {
    float: left;
    width: 406px;
    min-height: 30px;
    padding-left: 8px;
  ...
  .twoverify-right-item .checkbox-group span em {
    color: #d6dff9;
  }

  .twoverify-right-item .checkbox-group span a {
    color: #fdb902;
    text-decoration: underline;
  }

  .twoverify-right-item .checkbox-group span a:hover {
    color: #fdb902;
  }

  .twoverify-right-item .button-group {
    position: absolute;
    bottom: 40px;
    left: 60px;
    width: 428px;
  ...
  .twoverify-right-item .button-group /deep/ .button.disabled {
    background-color: #999;
  }

  .twoverify-right-item .button-group .link {
    padding-top: 6px;
    overflow: auto;
  }

  .twoverify-right-item .button-group .link a {
    height: 24px;
    line-height: 24px;
    color: #fdb902;
    text-decoration: underline;
  ...
  .twoverify-right-item .button-group .link a:first-of-type {
    float: left;
  }

  .twoverify-right-item .button-group .link a:last-of-type {
    float: right;
  }

  .twoverify-right-item .button-group .link a:hover {
    color: #fdb902;
  }

  .twoverify-right-item /deep/ input::-webkit-input-placeholder {
    font-size: 14px;
    color: #404b69;
  }

  .twoverify-right-item /deep/ input::-moz-placeholder {
    font-size: 14px;
    color: #404b69;
  }
  ...
  .twoverify-right-item /deep/ input:-ms-input-placeholder {
    font-size: 14px;
    color: #404b69;
  }
  ...
  .verifyCode /deep/ ul li.active {
    border-bottom-color: #3A76E7;
  }

  .verifyCode /deep/ ul li:last-child {
    margin-right: 0;
  }
  ...
  .sendVerifyCode.disabled, .sendVerifyCode.disabled:hover {
    color: #999;
    cursor: not-allowed;
  }
</style>
