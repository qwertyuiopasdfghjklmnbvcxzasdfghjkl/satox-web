<template>
  <div ref="withdrawBox" class="withdrawBox" @click="showDropdown=false">
    <div class="koall-verify-all">
      ...
      <div class="f-fl">
        <div class="filed" v-if="(symbol==='USSD' && !ussdWithdrawKey) || (symbol==='USDT' && usdtWithdrawKey === 2)">
          <em>
            {{$t('public.account_info')}}<!--账户信息--> ：<i class="asterisk">&nbsp;*</i>
          </em>
          <div class="bank-info" style="position:relative;">
            <input type="text" maxlength="100" v-validate="'required'" data-vv-name="bankCountryCode"
                   v-model="userBankInfo.bankCountryCode" :placeholder="$t('public.state_bank')"
                   :class="{error:errors.has('bankCountryCode')}"/>
            <input type="text" maxlength="100" v-validate="'required'" data-vv-name="bankProvince"
                   v-model="userBankInfo.bankProvince" :placeholder="$t('public.bank_province')"
                   :class="{error:errors.has('bankProvince')}"/>
            <input type="text" maxlength="100" v-validate="'required'" data-vv-name="bankDistrict"
                   v-model="userBankInfo.bankDistrict" :placeholder="$t('public.bank_area')"
                   :class="{error:errors.has('bankDistrict')}"/>
            <input type="text" maxlength="100" v-validate="'required'" data-vv-name="bankCode"
                   v-model="userBankInfo.bankCode" :placeholder="$t('public.bank_number')"
                   :class="{error:errors.has('bankCode')}"/>
            <input type="text" maxlength="100" v-validate="'required'" data-vv-name="bankBranch"
                   v-model="userBankInfo.bankBranch" :placeholder="$t('public.branch_name')"
                   :class="{error:errors.has('bankBranch')}"/>
            <input type="text" maxlength="100" v-validate="'required'" data-vv-name="bankRealname"
                   v-model="userBankInfo.bankRealname" :placeholder="$t('public.account_name')"
                   :class="{error:errors.has('bankRealname')}"/>
            <input type="text" maxlength="100" v-validate="'required'" data-vv-name="bankNumber"
                   v-model="userBankInfo.bankNumber" :placeholder="$t('public.bank_account')"
                   :class="{error:errors.has('bankNumber')}"/>
          </div>
        </div>
        <div class="filed"
             v-if="(symbol!=='SATO' || (symbol==='SATO' && !satoWithdrawKey)) && (symbol!=='USDT' || (symbol==='USDT' && usdtWithdrawKey === 0)) && symbol!=='USSD'">
          <em>
          ...
          <div class="withAdress" style="position:relative;" :class="{error:errors.has('selToAddress')}">
            <input type="text" maxlength="150" v-if="!showNewAddress" v-validate="'required'"
        ...
        <div class="filed" v-show="showNewAddress" style="position:relative;">
          ...
          <input class="address" type="text" maxlength="150" v-validate="'required'" data-vv-name="toAddress"
        ...
        <div class="filed">
          ...
          <template v-if="symbol==='USSD'">
            ...
            <span class="large_withdraw" @click="ussdWithdrawKey=true"
                  v-else>{{$t('account.withdraw_to_satoken')}}</span>
          </template>
          <template v-if="symbol==='SATO'">
            ...
            <span class="large_withdraw" @click="satoWithdrawKey=true"
                  v-else>{{$t('account.withdraw_to_satoken')}}</span>
          </template>
          <template v-if="symbol==='USDT'">
            <!-- <template v-if="false"> -->
            <span class="large_withdraw" @click="usdtWithdrawKey=0" v-show="usdtWithdrawKey !== 0">{{$t('account.withdraw_to_wallet')}}</span>
            <span class="large_withdraw" @click="usdtWithdrawKey=1" v-show="usdtWithdrawKey !== 1">{{$t('account.withdraw_to_satoken')}}</span>
            <span class="large_withdraw" @click="usdtWithdrawKey=2" v-show="usdtWithdrawKey !== 2">{{$t('account.withdraw_to_card')}}</span>
          </template>
        </div>
      </div>
      <div class="f-fr">
  ...
  export default {
    ...
    computed: {
      ...
      procedureFee () { // 手续费 提现数量-固定手续费
        ...
        } else if (this.symbol === 'USDT' && this.usdtWithdrawKey === 2) {
          return utils.removeEndZero(numUtils.mul(this.amount, 0.1).toFixed(8))
        } else if ((this.symbol === 'USSD' && this.ussdWithdrawKey) || (this.symbol === 'SATO' && this.satoWithdrawKey) || (this.symbol === 'USDT' && (this.usdtWithdrawKey === 0 || this.usdtWithdrawKey === 1))) {
          return '0'
        } else {
    ...
    methods: {
      ...
      walletWithdraw () { // 提现请求
        ...
        if ((this.symbol === 'USSD' && !this.ussdWithdrawKey) || (this.symbol === 'USDT' && this.usdtWithdrawKey === 2)) {
          let userBankInfo = {
        ...
        } else if ((this.symbol === 'SATO' && this.satoWithdrawKey) || (this.symbol === 'USSD' && this.ussdWithdrawKey) || (this.symbol === 'USDT' && this.usdtWithdrawKey === 1)) {
          this.withdrawType = 1
        } else {
          this.withdrawType = 0
        }
            ...
            okCallback: (code) => {
              ...
              let saveFun = () => {
                userUtils.walletWithdraw(formData, () => {
                  let msg = typeof code === 'object' && code.type === 1 ? this.$t('message.WITHDRAWALS_SUCCESS') : (this.getSysParams.withdrawEmail && this.getSysParams.withdrawEmail.value === '1' ? this.$t('login_register.Mail_sent_successfully') : this.$t('message.WITHDRAWALS_SUCCESS'))
...
<style scoped>
  ...
  .withdrawBox .filed .withAdress {
    ...
    height: 30px;
    background-color: #FFF;
    border: 1px solid rgba(186, 141, 53, 0.5);
  }
  ...
  .bank-info input:nth-of-type(3n) {
    margin-right: 0;
  }

  .bank-info input:last-of-type {
    width: 502px;
    margin-right: 0;
    margin-bottom: 0;
  }
