              ...
              <ul class="list1">
                  <li v-for="data in curProducts" :key="data.marketId" class="list1-item" :class="{'list1-item-active':data.market === symbol}" @click="changeSymbol(data)">
                      ...
                      <span class="list1-col list1-currency">
                          ...
                          <span class="fs12">{{$t(`symbol.${data.currencySymbol}`)}}</span>
                      </span>
...
export default {
  ...
  computed: {
    ...
    markets () {
      // 收藏
      return ['collection'].concat([...new Set(this.products.map(item=>{return item.baseSymbol}))])
    },
    tipPrice () {
  ...
  beforeDestroy () {
    this.removeEvents('commodityMarketEvent')
  },
  methods: {
...
.list1-currency{display:flex;color:#333;flex-flow:column;align-items:flex-start;justify-content:center; max-width:70px;margin-left:10px; margin-right:10px;}
