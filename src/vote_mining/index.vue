      ...
      <div class="content">
        <div class="join-form" :class="{en:getLang==='en'}">
          <div class="form-item flex">
            ...
            <p>
              <input type="text" name="community" readonly="" v-model="formData.communityId"
                     :placeholder="$t('business.CHOOSE_YOUR_COMMUNITY')" @click="showCommunities=!showCommunities">
            <ul class="communities" v-show="showCommunities">
              <li v-for="item in voteInfo.communities"
                  @click="formData.communityId=item.communityName;showCommunities=false">{{item.communityName}}
              </li>
            </ul>
            </p>
          </div>
          <div class="form-item flex">
            ...
            <p><input type="number" name="mobile" v-model="formData.invitePhone"
                      :readonly="voteInfo.invitePhone?true:false"
                      :placeholder="$t('business.INVITE_MOBILE_PLACEHOLDER')"></p>
          </div>
          <div class="form-item">
            <p><span>*</span>{{$t('business.TICKET_PERIOD')}}：</p>
            <ul class="period">
              <li v-for="period in voteInfo.periods" :class="{active:period.votePeriodId===formData.periodId}"
                  @click="formData.periodId=period.votePeriodId">
                <i></i>
          ...
          <div class="mt45 fs32 text-center gray">
            {{$t('business.NOT_VOTE')}}
            <router-link :to="{name:'vote_mining_profile'}" tag="span" class="blue pointer">
              {{$t('business.VIEW_EARNINGS')}}
            </router-link>
          </div>
        </div>
        <div class="rules">
          ...
          <article class="mt45 fs26">
            ...
            <p>2. {{$t('business.ACTIVITY_RULE_2')}}<br/>{{$t('business.ACTIVITY_RULE_2_a')}}<br/>{{$t('business.ACTIVITY_RULE_2_b')}}
            </p>
            <p>{{$t('business.ACTIVITY_RULE_2_b_n')}}</p>
            <p>3. {{$t('business.ACTIVITY_RULE_3')}}<br/>{{$t('business.ACTIVITY_REMARK_1')}}<br/>{{$t('business.ACTIVITY_REMARK_2')}}
            </p>
          </article>
        </div>
...
<style scoped>
  ...
  .form-item p input[name='community'] {
    background: url('../assets/images/vote_mining/arrow-down-blue.png') no-repeat right center;
    padding-right: 40px;
  }
  ...
  .period li:last-of-type {
    border-bottom: none;
  }

  .period li span:nth-of-type(2) {
    flex: 1;
    background: url('../assets/images/vote_mining/dot_bg_gray.png') repeat-x center;
    margin-left: 10px;
    margin-right: 10px;
  }
  ...
  .period li.active span:nth-of-type(2) {
    background-image: url('../assets/images/vote_mining/dot_bg_blue.png');
  }
