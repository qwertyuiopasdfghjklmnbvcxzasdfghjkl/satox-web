    ...
    <div class="header-fixed">
      <div class="header-center"
        ...
        <div class="center-left">
          <ul class="maxWidth">
            <li>
              <a @click="reloadPage('exchange_index')"
            ...
            <li class="message" @mouseover="showPanel" @mouseout="hidePanel">
              ...
              <div class="message-panel" :class="{'message-panel-show':showMessage}">
                ...
                <div class="message-panel-container"
                     :class="{'all-center':!isLogin || (isLogin && messageList.length === 0)}">
                  <div class="message-panel-list" v-if="isLogin && messageList.length > 0">
                    <div class="message-panel-list-item" :class="{'message-readed':item.messageState === 1}"
                         v-for="(item, index) in messageList" :key="index" @click="goPage(item)">
                      <p class="message-panel-list-item-title">{{formatSystemMessage(item.title, item.msgType !==
                        5)}}</p>
                ...
                <div class="message-panel-operation" v-if="isLogin">
                  ...
                  <span class="op-mark" :class="{disabled: unReadLength === 0}" @click="markAllMsg">{{$t('message.all_read')}}
                    <!--全部标记为已读--></span>
                </div>
              </div>
            </li>
...
<style scoped>
  ...
  .message-panel-arrow {
    ...
    border-style: solid;
    border-color: transparent transparent #404b69 transparent;
  }

  .message-panel-arrow::after {
    content: "";
    display: block;
    width: 0;
    height: 0;
    ...
    border-style: solid;
    border-color: transparent transparent #181b2a transparent;
  }
  ...
  .message-panel-container::-webkit-scrollbar {
    width: 5px;
    height: 5px;
  ...
  /*定义滚动条轨道 内阴影+圆角*/
  .message-panel-container::-webkit-scrollbar-track {
    -webkit-box-shadow: inset 0 0 2px rgba(0, 0, 0, 0.3);
    background-color: #555;
  }

  /*定义滑块 内阴影+圆角*/
  .message-panel-container::-webkit-scrollbar-thumb {
    -webkit-box-shadow: inset 0 0 2px rgba(0, 0, 0, .3);
    background-color: #8e8e8e;
  }

  .message-panel-list-item {
    padding: 2px 18px;
    cursor: pointer;
  }

  .message-panel-list-item:hover {
    background-color: #566a96;
  }

  .message-panel-list-item-title,
  .message-panel-list-item-time {
    height: 24px;
    line-height: 24px;
    font-weight: bold;
    white-space: nowrap;
  ...
  .message-readed .message-panel-list-item-time {
    font-weight: normal;
    color: #3a435d;
  }

  .message-panel-container.all-center {
    display: flex;
    justify-content: center;
    align-items: center;
  }
  ...
  .message-panel-nologin .nologin-icon {
    font-size: 60px;
    text-align: center;
  }

  .message-panel-nodata .nodata-text,
  .message-panel-nologin .nologin-text {
    font-size: 14px;
    color: #181b2a;
  }

  .message-panel-nologin .nologin-text a {
    color: #3283FF;
  }

  .message-panel-nologin .nologin-text a:hover {
    color: #3283FF;
  }

  .message-panel-operation {
    display: flex;
  ...
  .message-panel-operation span {
    font-size: 12px;
    color: #aeb7d0;
    cursor: pointer;
  }

  .message-panel-operation span:hover {
    color: #3283FF;
  }

  .message-panel-operation span.disabled {
    color: #999;
    cursor: not-allowed;
  }

  .message-panel-operation span.disabled:hover {
    color: #999;
  }
  ...
  .header-center .center-left > ul {
    height: 24px;
  }

  .header-center .center-left > ul li {
    float: left;
    margin-left: 12px;
    height: 24px;
    line-height: 24px;
  ...
  .header-center .center-left > ul li:first-of-type {
    margin-left: 54px;
  }

  .header-center .center-left > ul li a {
    padding-left: 6px;
    padding-right: 6px;
    font-size: 14px;
    color: #FFFFFF;
  ...
  .header-center .center-left > ul li a:hover, .header-center .center-left > ul li a.current {
    color: #3283FF;
  }
  ...
  .header-center .center-right > ul {
    height: 24px;
  }

  .header-center .center-right > ul > li {
    float: right;
    margin-left: 12px;
    height: 24px;
    line-height: 24px;
  ...
  .header-center .center-right > ul > li > a {
    padding-left: 6px;
    padding-right: 26px;
    font-size: 14px;
    cursor: pointer;
  ...
  .header-center .center-right > ul > li > a:hover {
    color: #BA8D35;
  }

  .header-center .center-right ul li.translations {
    position: relative;
    margin-left: 18px;
  }

  .header-center .center-right ul li.translations a {
    display: block;
    height: 22px;
    padding-left: 0;
    padding-right: 0;
  ...
  .header-center .center-right ul li.translations a.unfold {
    color: #3283FF;
    border-color: #3283FF;
  }

  .header-center .center-right ul li.translations a i {
    padding-left: 12px;
    font-size: 12px;
  }

  .header-center .center-right ul li.translations .lang-list {
    display: flex;
    flex-flow: column;
    width: 104px;
    position: absolute;
    ...
    box-shadow: 0 3px 6px #12141f;
  }

  .header-center .center-right ul li.translations .lang-item {
    flex: 1;
    height: 38px;
    line-height: 38px;
    font-size: 14px;
  ...
  .header-center .center-right ul li.translations .lang-item:hover {
    color: #cbd4ec;
    background-color: #252e44;
  }

  .header-center .center-right ul li.message {
    position: relative;
    margin-left: 0;
  }

  .header-center .center-right ul li.message > a {
    display: block;
    position: relative;
    height: 20px;
    padding-left: 0;
  ...
  .header-center .center-right ul li.message > a.unfold {
    color: #BA8D35;
  }

  .header-center .center-right ul li.message > a em {
    position: absolute;
    top: -5px;
    left: 11px;
    min-width: 6px;
  ...
  .header-center .center-right > ul.minWidth {
    display: none;
  }
  ...
  .header .minWidthList li {
    display: flex;
  }

  .header .minWidthList li a {
    display: flex;
    flex: 1;
    height: 24px;
    line-height: 24px;
  ...
  .header .minWidthList li.sepline {
  ...
  @media screen and (max-width: 1199px) {
    ...
    .header-center .center-left > a {
      background-size: 70%;
    }

    .header-center .center-left > ul.maxWidth,
    .header-center .center-right > ul.maxWidth {
      display: none;
    }
    ...
    .header-center .center-right > ul.minWidth > li {
      height: 40px;
      line-height: 40px;
    }

    .header-center .center-right > ul.minWidth .icon-market-list1 {
      font-size: 40px;
    }
